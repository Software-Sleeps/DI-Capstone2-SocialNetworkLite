...
class GetPosts extends Component {
  ...
  addLike(id) {
    ...
    this.setState((currentState) => {
      return { likedPosts: [...currentState.likedPosts, id] };
    });
  ...
  removeLike(id) {
    this.setState((currentState) => {
      ...
      const copy = [...currentState.likedPosts].splice(currentState.likedPosts.id)
  ...
  render() {
    ...
    const allPosts = this.state.posts.map((element, index) => {
      ...
      let button = this.state.likedPosts.find((likedPost) => {
        return likedPost === element.id 
      }) ? (<Button
